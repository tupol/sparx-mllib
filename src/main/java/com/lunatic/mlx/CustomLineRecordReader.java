    ...
    public boolean nextKeyValue() throws IOException {
        ...
        while (pos < end) {
            ...
            if (newSize < maxLineLength) {
                // if value starts with "{" then append all lines until next "{" or EOF
                if (value.toString().trim().startsWith("{") || nextLine.toString().trim().startsWith("{")) {
                    ...
                    // this is true after the first time a "{" line occurs
                    if (nextLine.toString().trim().startsWith("{")) {
                        Text swap = value;
                        value = new Text("");
...
//                    //in case there is only only one v between two "{" lines
//                    if (tmp.toString().trim().startsWith("{")) {
//                        nextLine = tmp;
//                    }
                    // while new line does NOT start with "{"
                    while (!tmp.toString().trim().startsWith("{")) {

                        if ( tmp.toString().trim().equals("},"))
                            tmp = new Text("}");
                        else if ( tmp.toString().trim().equals("]") && nextLine.toString().trim().equals(""))
                            tmp = new Text("");


                        //if meet the end of file, remove the last "]" character
                        if (tmp.toString().compareTo("") == 0) {
                            String valueStr =  value.toString();
                            value = new Text( valueStr.substring(0,valueStr.length() - 1));
                            break;
                        }
                        value.append(EOL.getBytes(), 0, EOL.getLength());
